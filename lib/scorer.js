 ...
 *
 * Signals (additive):
 *   +2  implement  — an explicit plan go-ahead ("确认，开始实施" / "proceed
 *                    with the plan"); waives both hard vetoes and is enough
 *                    on its own to reach the default threshold
...
/**
 * Explicit implementation commands — the "agent proposed a plan, user
 * green-lights it" flow. A hit is a strong signal (+2, sufficient on its own)
 * and waives both hard vetoes: these messages are typically short
 * ("确认，开始实施") yet carry the clearest possible intent.
 */
const IMPLEMENT_PHRASES = [
    '开始实施', '按方案实施', '照方案实施', '按方案执行', '照此方案',
    '按此方案', '确认实施', '同意实施', '方案通过', '动手吧', '开工吧',
    'start implementing', 'proceed with the plan', 'execute the plan',
    'implement the plan', 'go ahead with the plan',
];

/** Negation fragments that flip a nearby implement phrase into a refusal ("先别开始实施"). */
const NEGATORS = ['别', '不', '没', '莫', '勿', '尚未'];

/** @param {string} trimmed */
function hasImplementationCommand(trimmed) {
    const lower = trimmed.toLowerCase();
    for (const phrase of IMPLEMENT_PHRASES) {
        let from = 0;
        for (;;) {
            const at = lower.indexOf(phrase, from);
            if (at < 0) break;
            const before = lower.slice(Math.max(0, at - 4), at);
...
export function scoreGoal(text, options = {}) {
    ...
    const signals = [];
    const implement = hasImplementationCommand(trimmed);

    // An explicit implementation command waives the length floor: "确认，开始
    // 实施" is a handful of characters yet is the strongest trigger known.
    if (!implement && trimmed.length < minLength) {
    ...
    // --- vetoes --------------------------------------------------------------
    if (!implement && looksLikeQuestion(trimmed) && !hasStructure && !hasImperativeHint(trimmed)) {
    ...
    if (implement) {
        signals.push('implement(+2): explicit plan go-ahead');
        score += 2;
    }
